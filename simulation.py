...
def gen_corrupted_labels(delta_0, delta_1, y):
	Y_tilde = np.zeros(y.shape)
	for i in range(len(y)):
		if y[i] == 0:
			Y_tilde[i] = 1 if np.random.rand() < delta_0 else 0
		if y[i] == 1:
			Y_tilde[i] = 0 if np.random.rand() < delta_1 else 1
	return Y_tilde
...
def run_test(p = .5, delta_0=0, delta_1=None, n_examples=100, n_noise_runs=5):
	...
	plt.savefig(str(p) + "_" + str(delta_0) + "_" + str(delta_1) + "_" + str(n_examples) + ".png")
	plt.clf()
